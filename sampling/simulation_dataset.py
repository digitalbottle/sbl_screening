...
def prior_transform_3d_target(uniform_sample):
  ''' 
  ...
  priors_list = [priors.TopHat(mini=0.8, maxi=1.2),
                 ...
                 priors.TopHat(mini=pic_size[2]*0.3, maxi=pic_size[2]*0.7),
...
if __name__ == '__main__':
  ...
  for i in range(dict_size):
    I_3d, point = simulation_dictionary_generator(meshgrid=[xx, yy, zz], 
                                   psf=point_spread_function_3d, 
                                   theta_length=6, 
                                   prior_transform=prior_transform_3d_target)
    dict_list.append(I_3d.copy())
    point_list.append(point)
    img = std_Normalize(I_3d) * 255
    img = img.astype(np.uint8)
    imsave(os.path.join(dict_file, "%05d.tif"%i), img.transpose(2, 1, 0))
  ...
  for i in range(target_size):
    I_3d, I_3d_noise, points = simulation_data_generator(meshgrid=[xx, yy, zz],
                                  psf=point_spread_function_3d, 
                                  theta_length=6, 
                                  prior_transform=prior_transform_3d_target,
                                  noise_sigma=0.1,
    ...
    img = img.astype(np.uint8)
    imsave(os.path.join(target_file, "%05d.tif"%i), img.transpose(2, 1, 0))
