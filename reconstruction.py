...
for i in range(target_size):
  targets_clean_list.append(np.asarray(target_h5["targets_clean_%02d"%i]))
  target_list.append(np.asarray(target_h5["targets_noise_%02d"%i]))
...
for i in range(len(target_list)):
  taregt = tf.dtypes.cast(tf.reshape(target_list[i], [1, -1]), tf.float32)
  lambda_max = tf.reduce_max(tf.reduce_sum(dictionary_norm * taregt, axis=1))
  # ---------------------------------
  # Lasso
  # ---------------------------------
  ...
  w_pan, bg_pan = slr.pan_regression(lambda_pan, taregt, dictionary_norm, dict_list, point_list, target_point_list[0], save_path)
